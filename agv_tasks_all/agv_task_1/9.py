...
def compute_gradients(image):
    grad_x = cv.Sobel(image.astype(np.float32), cv.CV_32F, 1, 0, ksize=3)
    grad_y = cv.Sobel(image.astype(np.float32), cv.CV_32F, 0, 1, ksize=3)
        ...
        for _ in range(num_iter):
            ...
            for idx, point in enumerate(flow_points):
                ...
                if 0 <= nx < img1.shape[1] and 0 <= ny < img1.shape[0]:
                    intensity_diff = float(img2[ny, nx]) - float(img1[ny, nx])
...
# Parameters for Shi-Tomasi corner detection (tuned for motion tracking)
feature_params = dict(
    maxCorners=1000,   # Increased points for better detection
    qualityLevel=0.2,  # Lowered quality level for tracking more points
    minDistance=3,     
    blockSize=7,
    useHarrisDetector=True  # Harris corners improve tracking on dynamic objects
)
...
prev_gray = cv.GaussianBlur(prev_gray, (5, 5), 0)  # Noise reduction

# Motion-based mask creation
mask = np.zeros_like(prev_gray)
mask[50:450, 20:350] = 255  # Expanded mask to fully cover the person
